...
def AuthStatic(path):
    ...
    Serve Auth Static File
        ...
        todo:
            Later add nginx config for serve static files via nginx for user
    """
...
def admin_login_get(token):
    """
    Admin Login View
     - check admin token is valid and ok
    """
...
def admin_login_post(token):
    ...
    session['password'] = admin_db.Password
